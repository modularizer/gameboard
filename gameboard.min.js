...
// js/utils/mqtt-rtc.js
var baseTopic = (location.hostname + (location.hash || "")).replace(/[^a-zA-Z0-9]/g, ".");
  ...
  sync(data, sender) {
    if (data === "request") {
      ...
      for (let [name, item] of Object.entries(this.state.itemsByName)) {
        data2[name] = { position: item.position.toArray(), rotation: item.pivot.rotation.toArray() };
      }
      console.log("sending sync", data2, "to", sender);
...
var GameBoard = class extends HTMLElement {
  ...
  bindElements() {
    ...
    for (let game2 of this.gameNames) {
      let option = document.createElement("option");
      option.value = game2;
      option.innerHTML = game2;
  ...
  loadGame() {
    ...
    loadJSON(this.scene, src).then((({ models, metadata }) => {
      if (metadata.instructions) {
        let i2 = localStorage.getItem(game + "Instructions");
  ...
  handlers = {
    ...
    moves: (data, sender) => {
      console.log("Received moves from", sender, data);
    },
    audio: (data, sender) => {
      console.log("Received audio from", sender, data);
    },
